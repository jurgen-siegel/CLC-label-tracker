...
if page == "Manage Tickets":
    ...
    # Update button
    st.button('Update Ticket', key='update_ticket_button', on_click=st.session_state.update_ticket_pressed = True)
    ...
    if manage_option == "Add New Ticket":
        ...
        if not df.empty:
            ...
            st.button('Update Ticket', key='update_ticket_button',
                      on_click=st.session_state.update_ticket_pressed = True)
            if st.session_state.update_ticket_pressed:
                updated_data = {
            ...
            if not df.empty:
                ...
                def generate_html_table(dataframe):
                    ...
                    for _, row in dataframe.iterrows():
                        checks_count = row[
                            ['Artwork Received', 'Physical Proof', 'Digital Approved', 'Sample', 'Quote']].sum()
                        if checks_count == 1:
                ...
                df['in_progress'] = df[
                    ['Artwork Received', 'Physical Proof', 'Digital Approved', 'Sample', 'Quote']].sum(
                    axis=1).between(2, 4)
                df['just_started'] = df[['Artwork Received', 'Physical Proof', 'Digital Approved', 'Sample',
                                         'Quote']].sum(
                    axis=1) == 1
